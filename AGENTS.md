...
- **2026-09-25 (Sprint 36 / chat-real-llm-coverage Phase H + Wave 3 Phase 2 D4 立项 + archive, ship cycle)**: DECISION B(c) + DECISION C ship + Oracle Stage 2 SHIP-with-fixes + 4-file archive 完成. 4 commits on main: baseline `786d8cc` (3 files, +462/-30: tests/test_react_loop_real_llm.cpp 2→7 cases + proposal.md status PLACEHOLDER→🔍 Proposed + tasks.md full TDD 5-step progress) → SHIP-with-fixes `424c45f` (3 files, +150/-57: Oracle Stage 2 verdict ses_f2b41e215 Major 1 R4/R5 scope clarification + scope note comment + Minor 1 header comment fix + Minor 2 D6 median latency REQUIRE enforced + new `.rddf/improvements/real-llm-loop-level-e2e-followup.md` per Pattern #10 hygiene) → `e643dae` (9 files archive: DECISION B(c) git mv to `archive/2026-09-25-chat-real-llm-coverage-phase-h/` + DECISION C git mv to `archive/2026-09-25-wave-3-phase-2-d4-lora-pipeline/`, per Day 5 4-file integrity). DECISION B(c) ship detail: F1 §5 降级遗留 6 cases 真实 LLM 路径覆盖 (`test_react_loop_real_llm.cpp` 2→7 cases, 24→25 assertions, 100/100 stress test calls success). 断言强度分层 per AGENTS.md Pattern #3: R1/R2/R4 strict (precise content assertion) + R3 relaxed (stateless generate() 不共享 cross-call context, ChatSession multi-turn deferred) + R5/R6 capability (≥1/3 branches ok, ≥95% success rate + median latency ≤5s). R3 discovery (per Pattern #1 step 4): initial design "Alice in turn 2" strict assertion → 真实 DEEPSEEK 返回 "I don't know your name" → relaxed + documented limitation. DECISION C ship detail: Wave 3 Phase 2 D4 LoRA training pipeline 立项 (5 文件: `.openspec.yaml` + `proposal.md` + `design.md` + `tasks.md` + `specs/wave-3-phase-2-d4/spec.md`). 触发 24h cooling-off (Wave 3 Phase 1 merge `f0a5c4b` 2026-09-23T05:35Z → EXPIRED 2026-09-24T05:35Z ✅, 本 change 2026-09-25 → 2026-09-26). 范围建议 per Oracle: D4-D7 不要一个 change 全包, 先立 D4 (LoRA 训练管线) 为独立 change. Test 验证: `ctest -R test_react_loop_real_llm` 25 assertions / 7 cases / ALL PASS + R6 stress test 100 calls 100% success + median latency ≤5s. SHIP-with-fixes flow per AGENTS.md Pattern #4 (atomic commits, NOT amend). DECISION B(c) Oracle Stage 2 verdict Major 1 fix: R4/R5 over-claimed coverage (provider-level dispatch smoke labelled as loop-level E2E) — naming + scope clarification applied, loop-level E2E deferred to `.rddf/improvements/real-llm-loop-level-e2e-followup.md` (per Pattern #10 hygiene tracking). Pattern #10 hygiene: 4 项 deferred 全部建档 — 2 PLACEHOLDER archived + improvement entries, 1 umbrella already-shipped 校正, 1 real loop-level E2E improvement entry.
  - [Reverse Indicator for 786d8cc]: `+ new_up: F1 §5 降级遗留 6 cases 真实 LLM 路径覆盖 (2→7 cases, 24 assertions)` / `- old_down: drop_ratio=0% (纯 test coverage gap 回填)` / `failure_traces: 7/7 PASS 实测验证 (含 R3 relaxed 边界记录)` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for 424c45f]: `+ new_up: SHIP-with-fixes closure (scope clarification + follow-up tracking)` / `- old_down: drop_ratio=0% (test naming + doc fix, no production logic change)` / `failure_traces: 7/7 PASS (25 assertions after D6 latency REQUIRE)` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for e643dae]: `+ new_up: 2 OpenSpec changes 完成 archive (4-file 完整性保留)` / `- old_down: drop_ratio=0% (pure archive, no behavior change)` / `failure_traces: N/A` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for 65ff2dd]: `+ new_up: Wave 3 Phase 2 D4 立项 + 24h cooling-off 触发` / `- old_down: drop_ratio=0% (纯文档, 无代码变更)` / `failure_traces: N/A` / `ablation: N/A` / `context_ids: N/A`
- **2026-09-25 (Sprint 36 / ProviderLLMTool F1 Latent Site #3 defense-in-depth, SHIP-with-fixes + archive, ship)**: DECISION B(a) per Oracle `ses_f2b923412ffeTFfBdDqQFOMdT9`（项目重审 + DECISION 建议）。3 commits on main + Oracle Stage 2 SHIP-with-fixes 闭环 + Day 5 archive: baseline `c0d276a` (3 files, +372/-17: pdk/loop_agent/src/pdk_entry.cpp ProviderLLMTool `if (out.text.empty())` fail-fast guard + tests/test_provider_llm_tool_empty.cpp 3 cases/11 assertions new + tasks.md TDD 5-step plan) → SHIP-with-fixes `ed60230` (2 files, +87/-123: Oracle Stage 2 M1 Case 3 exact-string source guard fix 防假阴性 + M2 D4 acceptance 改自闭环记录 + M2b umbrella already-shipped 校正 + Mi1 ~30 TDD checkbox 补勾 + Mi2 D3 全量 ctest deferred 标注 + Mi3 D1 throw-not-produce-LLMResult 措辞修正) → archive `2cd4dce` (4 files: rename openspec/changes/{2026-09-18-provider-llm-tool-empty-passthrough => archive/2026-09-25-provider-llm-tool-empty-passthrough}, per Day 5 4-file integrity lesson). Oracle verdict: 0 Critical / 2 Major / 3 Minor. SHIP-with-fixes 流程 per AGENTS.md Pattern #4 (atomic commits, NOT amend). 同期 cleanup: (a) `1022d49` DECISION D — 2 PLACEHOLDER OpenSpec（`2026-09-17-fix-generate-subgraph-static-next` + `2026-09-17-intent-classification-router`）git mv 到 archive + CANCELLED notes，信息迁移至 `.rddf/improvements/` (per Day 5 4-file integrity lesson); (b) `ed8e956` CANCELLED notes append; (c) worktree `/workspace/project/wave-2-evidence-gate` 删除（Oracle 发现 `main..HEAD = 0`，evidence-gate 全部已 merged，DECISION A 假问题纠正） + branch `openspec/from-roadmap-phase-6c-evidence-gate` `-D` 删除 + `l2-evolution-deferred-follow-up/proposal.md` supersede 注记（Phase 1 R8 metrics 继承 evidence-gate 真实决议问题，防双轨实现）；(d) `l2-evolution-deferred-follow-up/` 5 文件新增（5 task validated ✅）—— 后续 Wave 4 sandbox + Wave 3 Ph2 D4-D7 + R8 full metrics + S4 research 路径立项载体. F1 Latent Sites 状态更新: Site #3 ✅ FIXED（自闭环记录在 change tasks.md §4.1，archive 冻结不改 per G2 precedent）；唯一真正 deferred 站点 `loop/process_task` 空 result 透传 (pdk_entry.cpp:402-450)，待独立 follow-up. Pattern #2 (≥3 sites → P0 promotion) 验证：umbrella `fix-generation-request-model-default` 已于 2026-09-08 archive ship 5 站点，本 change tasks.md:195 旧叙述"待评估"已纠正. Pattern #10 hygiene: 4 PLACEHOLDER/cancelled changes 用 git mv 而非删，确保 archived history 可追溯. L2 reference example (`pdk-chat-demo-evolution-reference-example`) 已 ship 8 commits + 9 L2 test binaries 323 assertions 全 PASS（验证报告：N1/N2/N5 硬限制 zero diff + 3 SoT docs §十一 sync）。Oracle ses_f2b923412 + ses_f2b5f9198 沉淀：worktree "763 files / NOT MERGED" 假象实为两点 diff 噪声（main 一个月 ship 在旧 worktree 视角显示"删除"），DECISION A 不需要 X/Y/Z/W 4 路线选择，直接关 worktree。Test 验证: ctest `test_provider_llm_tool_empty` 11/11 + test_dsl_engine_ctx_bridge 13/13 + test_executor 20/20 + test_executor_with_mock_provider 17/17 + test_chat_session_consumer 67/67 — 全 PASS。Pre-existing 与本 fix 无关: `test_loop_agent_plugin` 21/22 failed（libLoopAgent.so not found 环境问题，baseline stash 后验证同样失败）。Verification: git log HEAD~4..HEAD = 4 commits (c0d276a + ed60230 + ed8e956 + 1022d49 + da442c8)。
  - [Reverse Indicator for c0d276a]: `+ new_up: F1 Latent Site #3 defense-in-depth (空 text → fail-fast runtime_error)` / `- old_down: drop_ratio=0% (纯 fail-fast hardening, 正常路径不触发)` / `failure_traces: 空 text path → ProviderLLMTool → runtime_error → catch in call_llm_tool` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for ed60230]: `+ new_up: SHIP-with-fixes closure (M1 case guard + M2 acceptance rewrite + 3 Mi cleanup)` / `- old_down: drop_ratio=0% (test hardening + doc fix, no production logic change)` / `failure_traces: empty text path → ProviderLLMTool → runtime_error (still works)` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for 2cd4dce]: `+ new_up: DECISION B(a) 完成 archive (4-file 完整性保留)` / `- old_down: drop_ratio=0% (pure archive, no behavior change)` / `failure_traces: N/A` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for 1022d49]: `+ new_up: 8-day PLACEHOLDER info preserved in improvement tracker` / `- old_down: drop_ratio=0% (pure doc archive, no behavior change)` / `failure_traces: N/A` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for c0d276a]: `+ new_up: F1 Latent Site #3 defense-in-depth (空 text → fail-fast runtime_error)` / `- old_down: drop_ratio=0% (纯 fail-fast hardening, 正常路径不触发)` / `failure_traces: 空 text path → ProviderLLMTool → runtime_error → catch in call_llm_tool` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for ed60230]: `+ new_up: SHIP-with-fixes closure (M1 case guard + M2 acceptance rewrite + 3 Mi cleanup)` / `- old_down: drop_ratio=0% (test hardening + doc fix, no production logic change)` / `failure_traces: empty text path → ProviderLLMTool → runtime_error (still works)` / `ablation: N/A` / `context_ids: N/A`
  - [Reverse Indicator for 1022d49]: `+ new_up: 8-day PLACEHOLDER info preserved in improvement tracker` / `- old_down: drop_ratio=0% (pure doc archive, no behavior change)` / `failure_traces: N/A` / `ablation: N/A` / `context_ids: N/A`
- **2026-09-24 (Sprint 36 / pdk-chat-demo-deduplicate ship + L2 reference example complete, ship)**: 当日 2 个独立 ship。
  - **pdk-chat-demo-deduplicate** (archived `2026-09-24-pdk-chat-demo-deduplicate/`, 6 commits on main): HEAD = `da442c8` (HEAD of HEAD)。删除 dead code `tools/provider_switch_stub.{h,cpp}`（零调用方）commit `431d2d0`；新增 `SessionManager::dir()` / `current_session_id()` 公开 getter（消除 `tools/session_clone.cpp` 对内部成员 `dir_`/`current_session_id_` 的直接访问）commit `0d27fb6`；refactor session_clone 用 getter commit `fa3dd6b`；接线 `/compact` command 到 `SessionManager::compact()` (core 已存在，替代 "Compaction not yet wired" placeholder) commit `f29983e`；DESIGN.md dormant state 同步 + README contribution guide commit `9b30781`；8 D1-D8 follow-ups 在 `.rddf/improvements/pdk-chat-demo-followups.md` 注册 + index commit `da442c8`（D6 即时完成；D2 factory_slot 注入 + /model 升级 P2 真实可用性缺口优先；D1/D3-D5/D7/D8 全部 P3 触发时机 on-demand）。Ship 后审查发现 Pattern #10 hygiene 风险：D6 缺少可执行追踪机制 = D1-D5 散落 proposal/design/README 必遗忘，本 change 即时建档。
  - **pdk-chat-demo-evolution-reference-example** (L2 reference example, archived `2026-09-24-pdk-chat-demo-evolution-reference-example/`, 8 commits on main): 4-batch progressive execution with intermediate Oracle validation per AGENTS.md Pattern #11。Batch 1 `a380ec7` (skeleton + hermetic_home + context_request) + SHIP-with-fixes `e3d182f` (M1-M4 Oracle fixes); Batch 2 `ad2f42c` (evolution_tracer + evolution_session + T6.8a closure) + SHIP-with-fixes `9b9967f` (M1-M2 + m1); Batch 3 `e5098c1` (R9 anti-cheat + R8 reverse-indicators tests) + SHIP-with-fixes `14dccde` (M1-M2); Batch 4 `cc8739d` (main.cpp + run_evolution_demo.sh + README.md); final `960c5f5` (3-doc SoT §十一 sync + archive). 9 L2 test binaries / 39 cases / 323 assertions 全 PASS, N1/N2/N5 硬限制 zero diff verified, hermetic HOME preserved (`/tmp/l2-test-<uuid>`), 4 ADR-0068 v2.4 events registered (`mutation_metric_rejected` + `turn_input_network_keyword_rejected` + `hidden_context_accepted_info` + `hint_containment_rejected`), `eval_quality` always null per P0-4 C5 invariant, attribution_verdict distribution Baseline → Attributed, session_id consistent. 14 fixtures (3 SHIPPED + 11 test) JSON-legal。L2 deferred items D1.1-D1.7 documented in design.md → 后续 `l2-evolution-deferred-follow-up/` change carrier.
- **2026-09-23 (Sprint 36 / Wave 3 Phase 1 finetune-base-model SHIP-with-fixes cycle, ship)**: Wave 3 Pilot 激活 (per Pre-Wave3 Plan §3 + ADR-0078 翻牌 + 24h cooling-off override per user-explicit HARD pause). 经 Sisyphus-Junior async worker (bg_8cb82c2c, 1h58m) + Oracle SHIP-with-fixes 复评 (bg_7fe026cc, 49m) 完整 ship 3 commits on worktree + merge: impl `97a2abb` (18 files, +1057/-62: 4 件套 + ADR-0078 翻牌 + D1 评分 yaml + D3 脚本 + D7 stub + 2 tests + archive 5 文件) → SHIP-with-fixes (1 Critical + 1 Major + 4 Minor) → SHIP-with-fixes commit `232eb13` (5 files, +66/-25: 6 test 断言 + D1 算术 5/5 + ADR-0078 D1 NOTE) → merge `f0a5c4b`. 主会话 Mi1/Mi3 接手 (AGENTS.md + builder-handoff git-track + post_impl_execute_summary):
  - **C1 (Critical) fix**: `LLMProviderFactory` ctor 自注册 `agenticdsl-llama-3.1-70b-lora-v1` (per design D7-3 + ADR-0078 Phase 1 容量评估) 污染 `dynamic_factories_`, 破坏 `tests/test_provider_factory.cpp:168/181/189` + `test_provider_factory_concurrent.cpp:63` + `test_provider_register_dynamic_tool.cpp:64/72` 共 6 处 `dynamic_names()` 断言. 修复: 6 处断言更新反映 ctor 自注册 baseline (size 从 0→1 / 1→2 / 2→3) + rationale 注释说明为何 ctor 非空是 design 显式决策非 regression + 设计决策注释块 (引用 design D7-3 + ADR-0078 Phase 1 容量评估). C1.1 实现偏差记录: 精确比较加 `std::sort` 前置 (避免 unordered_map 哈希序不确定性, 任务意图 "unordered_map 不保证顺序" 的正确实现).
  - **M1 (Major) fix**: D1 评分 yaml 5/5 候选 `weighted_score` 算术错误. 文档声称 vs 实际: gpt-4 6.0→**5.8** / claude 5.35→**5.25** / llama 8.0→**8.2** / qwen 8.25→**8.45** / deepseek 7.5→**7.35**. **最严重**: deepseek-v2-chat 实际 7.35 **< 7.5**, 不满足 Weighted ≥ 7.5 过滤条件, 但文档标 `passed_all_filters: true` 是 false positive. 修复: 5 个 weighted_score 全部修正 + deepseek `passed_all_filters: false` + 4 过滤条件验证表 deepseek `❌ 7.35` + 校正说明段 (含最终选择 llama 不变论证: 8.2 ≥ 7.5 ✅) + ADR-0078 D1 段同步修正 + 段末"算术校正 NOTE" (引用 Oracle bg_7fe026cc).
  - **Mi1 fix (本条目)**: AGENTS.md Recent Changes 同步
  ...
  - **设计依据**: Pre-Wave3 Plan §3 (`.rddf/roadmap/2026-09-21-pre-wave3-to-wave3-execution-plan.md`) + ADR-0078 v1.0 翻牌 ✅ Approved + Oracle bg_7fe026cc verdict (1 Critical + 1 Major + 4 Minor) + mode #11 (Async Worker + Dual Oracle dual-review SHIP-with-fixes cycle) 完整闭环. **下一步**: Wave 3 cooling-off (24h) → Wave 3 Phase 2 立项准备 (D4-D7 拆 OpenSpec change)
...
- **2026-09-22 (Sprint 35-36 / G2 evolution-verdict-reward-quality SHIP cycle, ship)**: Pre-Wave3 Plan §2 4-Gate 序列最后闸口. 经 Sisyphus-Junior async worker (`bg_a0ebaef9`, 44m 13s) 完整 ship 3 commits on worktree + merge `dc12a17` → Oracle post-impl 复评 (`bg_ebfe1c25`, 15m 29s) 判定 **SHIP (0 Critical + 0 Major + 2 Minor 不阻塞)**:
  - 3 commits: `e51073a` feat(evolution): EvolutionVerdict reward_quality field + harness_rsi.cpp wiring + 2 tests + design D1a 4→5 字段约束修订; `737e979` chore(openspec): archive (5-file integrity per AGENTS.md Day 5 lesson); `3cdb792` chore(governance): builder state FULL schema (post_impl_review_prompt 已写, 主会话可直派 Oracle)
  - **核心改动**: `transition_guard.h` `EvolutionVerdict` 加 `reward_quality: agenticdsl::RewardSignal::Quality` 字段 (默认 `Quality::Acceptable` per design D1; D2 注释 4→5 字段约束显式修订 per design D1a); `evaluate_readiness()` 填充 `verdict.reward_quality = reward.quality` 在 can_proceed 判定之前 (失败质量不丢失 per design D2); `harness_rsi.cpp:140` `evolution.readiness.denied` 事件 `eval_quality` 字段从硬编码 `"Unknown"` 改为 `agenticdsl::evaluation::quality_name(verdict.reward_quality)` (复用 `evaluation_events.h:28` 既有 helper, 不新增第三个同义函数 per design D3)
  - **Oracle 顺带发现 namespace 勘误**: design.md:57 D3 rationale 写 `agenticdsl::quality_name`, 实施用真实 namespace `agenticdsl::evaluation::quality_name` (design 笔误, builder.json design_deviations 已记录, archived design.md 冻结可接受)
  - **测试**: test_transition_guard 14/14 (51 assertions, +1 G2 case) PASS + test_harness_rsi_pilot 22/22 (115 assertions, Case 2 强化为 `"Poor"`) PASS + 6 回归测试 (gepa_phase2 / genome_registry / genome_walk_ancestors / tool_registry 三变体) 全 PASS — 8/8 focused 零回归
  - **全量 post-merge ctest**: 208/210 PASS (99%), 2 失败均为 pre-existing (test_skill_interpreter KI 7.S29-1 + test_pdk_plan_execute BAD_COMMAND build 后 PASS) — 零 G1/G3/G4 回归
  - **2 Minor (不阻塞, post-merge 顺手)**: (a) archived design.md:57 D3 namespace 笔误 — 已 record in builder.json, design 冻结可接受; (b) spec 2 个 Excellent 场景 (`评估返回 Excellent` + `Excellent 事件透传`) 无独立断言 — 机制同一枚举直通, Acceptable+Poor 路径已覆盖
  - **Step 1 hygiene (commit `98711e3`, 主会话):** Oracle bg_eda2d180 战略复审发现的 3 项遗漏同步 — AGENTS.md Recent Changes 补 G3 + G4 条目, `git rm openspec/changes/sync-pdk-contract-header/` 5 残留文件, Decision Record §5.1 法律地位由 "GO with post-hoc closure gate" → "GO closed (G1/G3/G4 SHIPPED, G2 待启)"
  - **设计依据**: Pre-Wave3 Plan §2 G2 (`.rddf/roadmap/2026-09-21-pre-wave3-to-wave3-execution-plan.md`) + Oracle bg_ebfe1c25 verdict (8 项实测验证) + Oracle bg_eda2d180 战略复审 (4-Gate 序列闭环 + 24h cooling-off + Wave 3 立项准备建议). **下一步**: 24h cooling-off (起点 2026-09-22 22:30 = G2 merge dc12a17) → 期间做 Wave 3 立项**准备** (读 ADR-0078 background + 写 improvement 5-segment 草稿, 不正式立项) → 冷却期满 + Oracle 复审 → rdd-arch 启动 ADR-0078 `finetune-base-model` OpenSpec 流程 (B/D 路径合流)
