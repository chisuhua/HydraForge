...
**优先级**: P0 | **来源**: Phase 6b W1, `execution-plane-wave2`
**阶段**: phase-6b | **分类**: execution-plane-wave2
**类型**: governance
**主题**: ADR-0073 状态同步 + 实施范围审计
...
- **In Scope**: 新建 `docs/adr/adr-0073-impl-scope-audit.md`、将 ADR-0073 状态更新为 `🟡 Partial` 并补充证据段与修订后的复审节点、更新 `docs/README.md` 中 ADR-0073 的状态行、把 `roadmap.md` 中 W1 相关证据措辞改为真实 Phase 6a 来源并标记 W1 完成、执行 ADR lint、文档漂移检查与 OpenSpec 校验。
- **Out Scope**: `ToolMetadata` V3 字段、`ToolSchemaValidator`、`ToolCoordinator` 校验层、`DECLARE_TOOL` V3 自动生成、`docs/specs/dsl.md` §6.2 重写；任何 C++ 代码变更。
