# sprint-7-tech-debt-followup Specification

## Purpose
2026-06-22 Oracle 深度审查 `tech-debt-cleanup-sprint-6` 4 个 commit (`7cc4239` / `6c5557c` / `9fa0364` / `7923b2a`) 决议: 4 commits 保持合入不回退 (33/33 ctest pass, git status clean),**不 archive** Sprint 6 change;但验收项系统性未达 — 🔴 scheduler fork 处理逻辑复制两处 (`topo_scheduler.cpp:161-167` ≡ `:636-642`,唯一 bug 风险) + 🟠 execute() 222 行 vs ≤60 (差 3.7 倍) + 🟠 engine.cpp 跨模块 10 include vs ≤3 + 🔴 15 个新测试 0 交付 + 🟠 plugin 测试 7 case 名称/范围不符 + 🟠 scheduler factory 死代码零调用;本 change 闭环偏离项让 Sprint 6 change 最终 archive (Oracle session `ses_112a9f9c5ffesqpYeefOBgMkjH`)。
## Requirements
