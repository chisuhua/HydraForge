...
## Purpose
Phase 5 Stage 1 Step 0 (C10, IP-001 §Step 0) — `ExecutionSession` 新增 per-module 持久化状态 (`std::map<std::string, nlohmann::json> module_states_` 独立成员),懒初始化(首次 `dsl_call` 访问时创建空 json),支持 Agent 通过 DSL 累积模块级数据,Session 3 Oracle 决议 Q1 (ModuleState 独立于 LayeredContext,避免污染 ADR-0008 已 ship 的 L1-L5 语义纯性)。
## Requirements
