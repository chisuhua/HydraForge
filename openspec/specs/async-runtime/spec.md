...
## Purpose
ADR-0030 V2 异步运行时架构(替代 V1 dual-layer 归档)— 基于 Slice 00 ship 的 Taskflow v4.0 + async_simple v1.4,Phase 2 入口,Sprint 12 主体实施,采用双层(短时 DAG 并行 + 长生命周期协程 LLM Token 流 + 用户审批 suspend)而非 std::jthread 全替代。
## Requirements
