...
- [ ] 2.1 修订 `docs/adr/adr-0031-execution-policy.md` §决策 1: 8 方法 → **5 虚函数** (Oracle 推荐版)
...
- [ ] 2.2 修订 §附录"议题5最小集成": 标记 SUPERSEDED (旧 8 方法设计)
- [ ] 2.3 添加 §新决策 6: YOLO 切换需用户确认对话框 (defense-in-depth)

---

## 3. 重写 stub 头文件 (Sprint 13 Day 1)

- [ ] 3.1 编辑 `include/agenticdsl/policy/iexecution_policy.h`:
...
- [ ] 3.3 验证: `cmake --build build` 编译通过 (因 stub 无生产消费者, 应零回归)

---

## 4. P1 实施 - 3 个默认 Policy (Sprint 13 Day 2-3)

- [ ] 4.1 新建 `src/common/policy/plan_policy.{h,cpp}`:
...
- [ ] 4.2 新建 `src/common/policy/agent_policy.{h,cpp}` **(默认)**:
...
- [ ] 4.3 新建 `src/common/policy/yolo_policy.{h,cpp}`:
...
- [ ] 4.4 新建 `src/common/policy/policy_factory.{h,cpp}`:
...
- [ ] 4.5 CMake: `src/common/policy/CMakeLists.txt` 新建 + 静态库 `agenticdsl_policy`

---

## 5. P1 验证 - 单元测试 (Sprint 13 Day 4)

- [ ] 5.1 新建 `tests/test_execution_policy.cpp`:
...
- [ ] 5.2 验证: `ctest -R test_execution_policy --output-on-failure` ≥ 6 pass
- [ ] 5.3 `cmake --preset tsan && ctest -R test_execution_policy` 0 race
- [ ] 5.4 提交 1: `git commit -m "feat(policy): rewrite IExecutionPolicy 5-method + Plan/Agent/Yolo + factory (Sprint 13 P1)"`

---

## 6. P2 实施 - sync callback 审批机制 (Sprint 13 Day 5-6)

- [ ] 6.1 新增类型 (Sprint 13 Day 5):
...
- [ ] 6.2 新建 callback 实现工厂 (Sprint 13 Day 5):
...
- [ ] 6.3 新建 `src/common/policy/approval_handler.{h,cpp}` (Sprint 13 Day 6):
...
- [ ] 6.4 YOLO 切换确认对话框 (Sprint 13 Day 6):
...
- [ ] 6.5 验证: `cmake --build build` + `ctest` 0 回归
- [ ] 6.6 提交 2: `git commit -m "feat(policy): ApprovalHandler sync callback + ModeConfig + YOLO confirmation (Sprint 13 P2)"`

---

## 7. P2 验证 - 审批机制测试 (Sprint 13 Day 7)

- [ ] 7.1 扩展 `tests/test_execution_policy.cpp`:
...
- [ ] 7.2 新建 `tests/test_mode_switch_dialog.cpp`:
...
- [ ] 7.3 验证: `ctest -R "test_(execution_policy|mode_switch)"` ≥ 8 pass
- [ ] 7.4 提交 3: `git commit -m "test(policy): approval handler + mode switch dialog tests (Sprint 13 P2)"`

---

## 8. 集成与 ship gate (Sprint 13 Day 8-9)

- [ ] 8.1 集成到 DSLEngine (Sprint 13 Day 8):
...
- [ ] 8.2 IInteractionBus 桥接 (Sprint 13 Day 9):
...
- [ ] 8.3 ctest + sanitizer 全绿:
  - [ ] 8.3.1 `ctest --output-on-failure` ≥ 47/47 + 新增 ≥ 11 PASS
  - [ ] 8.3.2 `cmake --preset tsan && ctest` 0 race
  - [ ] 8.3.3 `cmake --preset asan && ctest` 0 leak
- [ ] 8.4 `python3 tools/adr_lint.py docs/adr/` exit 0
- [ ] 8.5 `python3 tools/docs_drift_audit.py` 0 critical drift
- [ ] 8.6 `openspec validate 2026-06-26-adr-0031-p1p2-execution-policy` exit 0

---

## 9. 同步与归档 (Sprint 13 Day 10)

- [ ] 9.1 更新 `docs/roadmap-status.md` §一 Phase 3 行: 0% → 100% (P1+P2)
- [ ] 9.2 更新 `AGENTS.md` § Recent Changes 顶部追加 Sprint 13 P1+P2 ship
- [ ] 9.3 同步 PDK 头文件: `./scripts/sync-pdk.sh` (PDK_ABI_VERSION 不变, 但需重新生成 .h)
- [ ] 9.4 `openspec archive 2026-06-26-adr-0031-p1p2-execution-policy --yes`
- [ ] 9.5 同步 master plan §四 C3 行: 状态 → ✅ archived
- [ ] 9.6 提交 4: `git commit -m "chore(openspec): archive C3 + Sprint 13 P1+P2 ship (ADR-0031 完整)"`

---

## 验证检查清单 (C3 ship gate)

- [ ] 1. Oracle 3 决策全部应用 (5 方法 + sync callback + Agent 默认 + YOLO 确认)
- [ ] 2. ADR-0031 同步修订 (8→5 方法, §附录 SUPERSEDED)
- [ ] 3. stub 重写 (8→5 方法) + ModeConfig 值结构体
- [ ] 4. 3 个 Policy 实现 (Plan/Agent/Yolo) + factory
- [ ] 5. ApprovalHandler + sync callback 3 种实现 (stdin/event_bus/test)
- [ ] 6. ModeSwitchDialog (YOLO 切换确认)
- [ ] 7. ctest ≥ 47 + 11 新测试 全绿
- [ ] 8. ASan/TSan 100% clean
- [ ] 9. `adr_lint.py` + `docs_drift_audit.py` + `openspec validate` 全 exit 0
- [ ] 10. master plan C3 行状态更新 + OpenSpec archive