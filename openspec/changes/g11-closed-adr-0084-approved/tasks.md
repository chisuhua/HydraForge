...
- [x] **T0.1** Write check: 静态验证 ADR-0084 当前状态为 🔍 Proposed (grep 文档第 6-8 行)
- [x] **T0.2** Verify fail: 文档当前仍标 🔍 Proposed (符合预期, ship gate 未通过)
- [x] **T0.3** Implement: `docs/adr/adr-0084-mutation-governance-contract.md` §状态 章节:
...
- [x] **T0.4** Verify pass: `grep -E "(🔍 Proposed|✅ Approved)" docs/adr/adr-0084-mutation-governance-contract.md` 第 1-20 行显示 ✅ Approved
- [x] **T0.5** Commit: `docs(adr-0084): Approved - V1 gate-and-audit ship (G11 Closed)`

## Phase 1: 文档状态同步 (估时 0.2 sprint)

### T1.1 capability-map §二 G11 行更新

- [x] **T1.1.1** Write check: 静态验证 G11 行当前状态 (grep "G11" cap-map §二)
- [x] **T1.1.2** Verify fail: cap-map §二 G11 行仍标 🔍 Proposed
- [x] **T1.1.3** Implement: `docs/architecture/capability-application-map-2026-08.md` §二 G11 行:
...
- [x] **T1.1.4** Verify pass: `grep -E "G11.*Closed\|G11.*Proposed" docs/architecture/capability-application-map-2026-08.md` 显示 ✅ Closed

### T1.2 capability-map §三 B7 自进化基础状态同步

- [x] **T1.2.1** Write check: grep "B7" cap-map §三
- [x] **T1.2.2** Verify fail: B7 行 G11 状态仍写 "🔍 (ADR-0084 起草中)"
- [x] **T1.2.3** Implement: §三 B7 行:
  - "G11 🔍 (ADR-0084 起草中) 契约" → "G11 ✅ Closed (V1 code ship, 2026-08-26)"
- [x] **T1.2.4** Verify pass: grep 显示 B7 G11 状态 ✅

### T1.3 capability-map §八 §八.5 排期表

- [x] **T1.3.1** Write check: grep "ADR-0084 mutation-governance" cap-map §八.5
- [x] **T1.3.2** Verify fail: 排期表当前记录 "Sprint 25 W1 启动 + Sprint 26 末评审 + G11 ✅ Closed" (待完成项)
- [x] **T1.3.3** Implement: §八.5 排期行追加完成日期列 "2026-08-26 ship" 标记
- [x] **T1.3.4** Verify pass: grep 显示完成状态

### T1.4 capability-map §七 changelog + 头部版本

- [x] **T1.4.1** Write check: grep "v1.6.1\|v1.7" cap-map 头部
- [x] **T1.4.2** Verify fail: 头部仍 v1.6.1
- [x] **T1.4.3** Implement: 头部 v1.6.1 → v1.7 + 最后验证 2026-08-26 + §七 changelog 新增 v1.7 条目:
  ```
  ...
  ```
- [x] **T1.4.4** Verify pass: 头部 + §七 同步

### T1.5 self-evolution-architecture §四.2 + 头部版本

- [x] **T1.5.1** Write check: grep "G11\|ADR-0084" self-evolution-architecture
- [x] **T1.5.2** Verify fail: 当前 v1.1.1 + G11 状态未标 ✅ Closed
- [x] **T1.5.3** Implement:
  - 头部 v1.1.1 → v1.2 + 最后验证 2026-08-26
  - §四.2 评估/奖励/变异治理行追加 "G11 Closed (2026-08-26, ADR-0084 Approved + V1 ship)"
- [x] **T1.5.4** Verify pass: grep 显示 ✅ Closed

### T1.6 active-status.md G11 跟踪段

- [x] **T1.6.1** Write check: grep "issue #14\|T19 GEPA Phase 1 只读" active-status §一
- [x] **T1.6.2** Verify fail: 当前 G11 跟踪段仍写 "issue #14 保持 OPEN" + "T19 GEPA Phase 1 只读反思约束"
- [x] **T1.6.3** Implement: `docs/active-status.md` §一 G11 跟踪段:
...
- [x] **T1.6.4** Verify pass: grep 显示 issue #14 关闭声明

### T1.7 adr-implementation-status-gap-analysis §一 总计行

- [x] **T1.7.1** Write check: grep "总计" gap-analysis §一
- [x] **T1.7.2** Verify fail: 当前总计 72 ADR (含 G11 起草中注释)
- [x] **T1.7.3** Implement: 注释更新 "G11 ADR-0084 起草中" → "G11 ADR-0084 ✅ Approved (V1 ship 2026-08-26)" + 状态计数同步 (Approved +1)
- [x] **T1.7.4** Verify pass: grep 显示 ✅ Approved

### T1.8 docs/README.md §adr/ 表 ADR-0084 行

- [x] **T1.8.1** Write check: grep "adr-0084" docs/README.md
- [x] **T1.8.2** Verify fail: 当前仍标 🔍 Proposed
- [x] **T1.8.3** Implement: ADR-0084 行:
...
- [x] **T1.8.4** Verify pass: grep 显示 ✅ Approved

### T1.9 Phase 1 验证

- [x] **T1.9.1** `python3 tools/adr_lint.py` 通过
- [x] **T1.9.2** `python3 tools/docs_drift_audit.py` 通过 (无新增 CRITICAL)
- [x] **T1.9.3** Commit: `docs(cap-map+self-evolution+active-status): G11 Closed (ADR-0084 Approved + V1 ship)`

## Phase 2: GitHub Issue 关闭 + ship gate (估时 0.1 sprint)

- [x] **T2.1** Issue #14 audit trail 准备:
...
- [x] **T2.2** `gh issue close 14 --comment "..."` (使用 T2.1 准备的 audit trail)
- [x] **T2.3** Verify: `gh issue view 14 --json state` 显示 `CLOSED`
- [x] **T2.4** Final ship gate:
...
- [x] **T2.5** Commit: `chore(openspec): archive 2026-08-26-g11-closed-adr-0084-approved + issue #14 closed`
- [x] **T2.6** `openspec archive 2026-08-26-g11-closed-adr-0084-approved`
