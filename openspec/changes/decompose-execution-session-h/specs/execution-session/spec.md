# Spec Delta: execution-session (PIMPL-lite decoupling)

This is a delta spec for the `execution-session` capability, capturing the PIMPL-lite decoupling invariants added in Sprint 19 (commit set accompanying `decompose-execution-session-h` change).
