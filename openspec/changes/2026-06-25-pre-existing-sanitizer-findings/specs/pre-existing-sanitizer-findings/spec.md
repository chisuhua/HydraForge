# Spec: pre-existing-sanitizer-findings

> **承接 change**: `engine-include-decoupling` spec §"sanitizer-revalidation" 优雅降级条款
> **触发事件**: 2026-06-25 P2.5 ship gate 复验发现 2 pre-existing 测试失败

## Purpose

跟踪 2026-06-25 P2.5 ship gate 复验 (`cmake --preset asan/tsan`) 发现的 2 个 pre-existing 测试失败。本 spec 在 `2026-06-24-engine-include-final-decoupling` archive 后创建,作为治理债跟踪 change,确保 Sprint 10 ship gate 通过且不丢失 pre-existing 失败信息。
