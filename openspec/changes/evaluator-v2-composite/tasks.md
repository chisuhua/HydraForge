...
- [ ] **T0.1** Write failing test: `tests/test_evaluator.cpp` 扩展 ≥ 3 cases:
...
- [ ] **T0.2** Verify fail: 编译失败（`fatal error: 'behavioral_equivalence_evaluator.h' file not found`）
- [ ] **T0.3** Implement: `include/agenticdsl/cognitive/behavioral_equivalence_evaluator.h`:
  ```cpp
  ...
  ```
- [ ] **T0.4** Implement: `src/modules/cognitive/behavioral_equivalence_evaluator.cpp`
...
- [ ] **T0.5** Verify pass: 3 cases PASS + V1 12 cases 零回归
- [ ] **T0.6** Commit: `feat(cognitive): BehavioralEquivalenceEvaluator V2 - T14 integration (T0)`

## Phase 1: CompositeEvaluator 契约 (估时 0.3 sprint)

- [ ] **T1.1** Write failing test: `tests/test_evaluator.cpp` 扩展 ≥ 3 cases:
...
- [ ] **T1.2** Verify fail: 编译失败（无实现）
- [ ] **T1.3** Implement: `include/agenticdsl/cognitive/composite_evaluator.h`:
  ```cpp
  namespace agenticdsl {
  ...
  ```
- [ ] **T1.4** Implement: `src/modules/cognitive/composite_evaluator.cpp`
...
- [ ] **T1.5** 注册 `src/modules/cognitive/CMakeLists.txt` 新源（**仅追加，不修改既有规则**）
- [ ] **T1.6** Verify pass: 3 cases PASS + 全部既有测试零回归
- [ ] **T1.7** Commit: `feat(cognitive): CompositeEvaluator V2 - multi-evaluator aggregation (T1)`

## Phase 2: 集成测试 (估时 0.2 sprint)

- [ ] **T2.1** Write failing test: `v1_v2_coexistence`（TaskSuccessEvaluator + BehavioralEquivalenceEvaluator 同时注入到 CognitiveWorker 互不干扰）
- [ ] **T2.2** Write failing test: `composite_with_v1_inside`（CompositeEvaluator 包装 TaskSuccessEvaluator → 验证 V1 在 V2 内可工作）
- [ ] **T2.3** Verify fail: 集成测试 FAIL（需要 worker 集成 mock）
- [ ] **T2.4** Implement: 测试内部使用现有 `set_evaluator(shared_ptr<IEvaluator>)` 注入多 evaluator（不修改 CognitiveWorker）
- [ ] **T2.5** Verify pass: 2 cases PASS
- [ ] **T2.6** Commit: `test(evaluator): V1+V2 coexistence + composite integration (T2)`

## Phase 3: 文档同步 + ship 验证 (估时 0.2 sprint)

- [ ] **T3.1** 修改 `docs/adr/adr-0083-evaluator-reward-contract.md` 头部 `##状态`:
  - 追加 V2 ship 证据段: `evaluator-v2-composite` change, BehavioralEquivalence + Composite 实施, 6+ cases PASS
- [ ] **T3.2** 修改 `docs/architecture/capability-application-map-2026-08.md`:
  - 头部 v1.9 → v2.0 + 最后验证 2026-08-27
  - §一 +1（新能力 #26 IEvaluator V2）
  - §八 评估信号行更新 V2 ship
  - §七 changelog 新增 v2.0 条目
- [ ] **T3.3** 修改 `docs/active-status.md` §一 G10 跟踪段：标注 V2 ship
- [ ] **T3.4** 验证: `python3 tools/adr_lint.py` 通过（≥82 ADR）
- [ ] **T3.5** 验证: `python3 tools/docs_drift_audit.py` 通过（无新增 CRITICAL）
- [ ] **T3.6** 验证: `ctest --output-on-failure` 全量 0 回归（动态基线，禁止硬编码 189）
- [ ] **T3.7** Commit: `feat(evaluator): ship IEvaluator V2 (BehavioralEquivalence + Composite) - V2 evaluation layer`
- [ ] **T3.8** `openspec archive evaluator-v2-composite`
