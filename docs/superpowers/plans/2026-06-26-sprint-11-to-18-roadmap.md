...
| 维度 | 状态 | 证据 |
|------|------|------|
| OpenSpec active change 数 | **1** (C8 `phase-4-5-mvp-cleanup`, 占位) | `ls openspec/changes/` = 1 active (C7 Phase 1+2 ship 后 archive; C8 待 C5+C6 后启动) |
| Test count | **61/61 PASS** | baseline 25 + Sprint 1a/1b/2/3/4/5/6/10/15/16/17/19/20 累计 36 新增 (含 C7: 4 new test files = interface + provider + policy + registry, 11 new TEST_CASE) |
...
| Phase 4 模型路由 | ✅ 100% | 2026-07-02 C7 fully shipped (Phase 1 + Phase 2: 3 strategy .so + ModelRegistry + examples + 11 new tests, ADR-0034 → ✅ Approved) |
| Phase 4.5 MVP清理 | 0% (待启动) | C8 占位, 待启动 |
...
| **C3** | `2026-06-26-adr-0031-p1p2-execution-policy` | 实施 | **1.5 周** (Sprint 13, Oracle 校正) | — | ✅ **shipped (2026-07-31, 39/39 ctest pass, 6 phase commits 230120e/acc7f55/76f52f9)** + ✅ **archived (2026-06-29, `openspec/changes/archive/2026-06-29-2026-06-26-adr-0031-p1p2-execution-policy/`)** |
...
| **C7** | `2026-06-26-adr-0034-model-router-plugin` | 实施 | 1-2 周 (Sprint 17) | — (PDK + PluginLoader 已 ship) | ✅ **fully shipped (2026-07-02, 61/61 ctest pass, Phase 1 + Phase 2 complete: 3 strategy .so + ModelRegistry + examples + 11 new tests, ADR-0034 → ✅ Approved)** |
...
| | ✅ **archived (2026-07-31)** — ADR-0030 V2 → ✅ Approved, C2 Day 1-10 ship: stream_to_bus bridge (Day 1-2) + Context fork/merge (Day 2-3) + P2 InMemoryBus EventBus MPMC (Day 6-8) + ADR doc fix (Day 9-10). C2 总 commit: e2fb89b + 4b1356a + d474b7d + 521ef3f |
...
| 2026-06-27 | C2 `adr-0030-v2-async-runtime` | Oracle 咨询完成 (session `ses_0f5541ebfffehKDxNVuYqB7bq4`): (1) Fleet 模式 16 路 LLM → DEFER (0 examples 使用并行 LLM, DomainWorkerPool 已提供 N-way 能力); (2) Token 流推送 → bridge runner `run_stream_to_bus` (IGenerationStream pull-based, Option A 侵入 provider 排除). C2 proposal/tasks/spec 全部 Oracle 决议应用. 估时 2 周 → 1.5-2 周. C2 实施 ship: stream_to_bus bridge + Context fork/merge + P2 InMemoryBus EventBus MPMC + ADR doc fix. 3 commits: e2fb89b / 4b1356a / d474b7d / 521ef3f | ✅ resolved |
