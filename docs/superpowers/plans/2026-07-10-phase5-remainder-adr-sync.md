...
| 2026-07-14 | C20-Spike (phase6-service-ification-v1) | **Metis RED + Oracle Q1-Q6 决策应用 (sessions `ses_0a1dd3355ffeUCmGTgLzyI1VXx` + `ses_0a17108b5ffexaXTWhF8vXot6b`)** | **C20 Spike framing (不兑现 ADR-0050 Candidate B)**: 创建 ADR-0051 (🔍 Proposed) 作为 Phase 6 内部 PDK 组合可行性 Spike; W1 fix list 11/12 ✅ (proposal/design/3 specs/tasks 全部重写 + 2nd Metis 0 CRITICAL + `openspec validate --strict` EXIT 0 + adr_lint EXIT 0 + docs_drift_audit 0 DRIFT); W2-W3 BLOCKED on Stage Gate 2026-07-18 + Sprint 23 capacity 1.5 eng × 2 周 + Oracle Q6 confirmation. ADR-0050 §决策 / §启动条件 **保持不动**; Phase 6 Candidate B v1 正式启动仍需满足 ADR-0050 §启动条件 5 项 | 🟡 active (W1 complete; W2-W3 待 unlock) |
| 2026-07-15 | **C19** (phase6-service-ification-v1 Spike) | **ship** | **C20-Spike W3 ship gate §10 11/11 ✅ PASS**: ADR-0051 ✅ Approved (experimental) + spike-onboarding.md ship + ToolCoordinator RAII guard + 5 escalation triggers (6/6 tests PASS) + Layer 3 dual memos + G1+G3 plugins (8 tests PASS) + E2E integration (3 tests PASS) + ctest **77/77 PASS 零回归** + ASan documented skip (0 code changes) + `openspec validate --strict` EXIT 0 + OpenSpec archived to `2026-07-15-phase6-service-ification-v1`. §12 (5 items) + §13 (7 items) deferred to Sprint 24+. **C20 kickoff: G2/G4/G5 teams use spike-onboarding.md as material**. 5 commits (d6e7762 ADR finalization + d4f0953 onboarding doc + b6effc5 ship gate + 086357c ADR flip + <this> archive+sync). | ✅ resolved (2026-07-15, C19 ship + archived) |
...
| 2026-07-14 | C20 Spike framing (内部 PDK 组合 vs 外部 MCP/OpenAI) | **Oracle Q1-Q6 决议 + Metis RED verdict 触发 reframing** | **ADR-0051 (🔍 Proposed) 创建**: Phase 6 内部 PDK Service Composition Spike (不兑现 ADR-0050 Candidate B); DECLARE_TOOL 弃用 (宏 `##name` 不支持 `.` / `/`) → `IToolRegistry::register_tool_function()`; 工具名改 `knowledge_base/query` (slash, ADR-0043); G3 `ToolCategory::Execute` + `{Workflow}` only (替代 ReadOnly); Layer 2 instrumentation 复用 `tool.audit.*` 事件 (禁止 `ToolRegistry::call_tool()` 全局注入); 5 escalation triggers 重分类为 2 runtime safety (ToolCoordinator RAII guard) + 2 plugin health (audit + G3 self-check) + 1 design review (Layer 1 + Layer 3 dual memos); `register_tool_function` 当前签名 `unordered_map<string,string> → nlohmann::json` (非 JSON-in/JSON-out); Spike → Candidate B 提升标准 5 项写入 ADR-0051 §后续 | **Oracle Q1-Q6 + 2nd Metis 复审 0 CRITICAL + `openspec validate --strict` EXIT 0** |
| 2026-07-15 | C19 Ship (phase6-service-ification-v1 Spike) | **W3 ship gate §10 11/11 PASS → ADR-0051 ✅ Approved (experimental) + OpenSpec archived** | **C20-Spike complete**: G1+G3 plugins (8 tests) + E2E integration (3 tests) + ToolCoordinator RAII + 5 escalation triggers (6 tests) + Layer 3 dual memos + spike-onboarding.md + ctest 77/77 PASS + ASan documented skip + `openspec validate --strict` EXIT 0 + `openspec archive` to `2026-07-15-phase6-service-ification-v1`. §12 (5 items Post-Ship) + §13 (7 items Promotion Criteria) deferred to Sprint 24+. C20 kickoff: G2/G4/G5 teams use spike-onboarding.md. | **5 commits (ADR finalization + onboarding + ship gate + ADR flip + archive+sync)** |
