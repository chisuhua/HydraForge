...
candidates:
  ...
  - name: claude-3-5-sonnet-20241022
    scores:
      ...
      openness: 2.0         # 闭源
    weighted_score: 5.25    # 9.5*0.3 + 6*0.2 + 3*0.2 + 2*0.3 = 2.85+1.2+0.6+0.6
  ...
  - name: llama-3.1-70b-instruct
    ...
    weighted_score: 8.2     # 8*0.3 + 5*0.2 + 9*0.2 + 10*0.3 = 2.4+1.0+1.8+3.0
  ...
  - name: qwen-2.5-72b-instruct
    ...
    weighted_score: 8.45    # 8.5*0.3 + 5.5*0.2 + 9*0.2 + 10*0.3 = 2.55+1.1+1.8+3.0
    passed_all_filters: true
  ...
  - name: deepseek-v2-chat
    ...
    weighted_score: 7.35    # 8.5*0.3 + 7*0.2 + 8*0.2 + 6*0.3 = 2.55+1.4+1.6+1.8
    passed_all_filters: false   # 实际 7.35 < 7.5, 不满足 Weighted ≥ 7.5 (算术校正见下方说明)
```
...
|----------|:---:|:---:|:---:|:---:|:---:|:---:|
| gpt-4-turbo-2024-XX | ❌ 5.8 | ✅ 9.0 | ❌ 3.0 | ❌ $10-30 | ✅ | ❌ |
| claude-3-5-sonnet-20241022 | ❌ 5.25 | ✅ 9.5 | ❌ 2.0 | ❌ $3-15 | ✅ | ❌ |
| **llama-3.1-70b-instruct** | ✅ 8.2 | ✅ 8.0 | ✅ 10.0 | ✅ 电费 | ✅ | ✅ |
| **qwen-2.5-72b-instruct** | ✅ 8.45 | ✅ 8.5 | ✅ 10.0 | ✅ 电费 | ✅ | ✅ |
| **deepseek-v2-chat** | ❌ 7.35 (Weighted < 7.5) | ✅ 8.5 | ✅ 6.0 | ✅ $0.14-0.28 | ✅ | ❌ |

## 最终选择

**`llama-3.1-70b-instruct`** (Weighted = 8.2, 4 过滤条件全过)

**论证**:
1. **Weighted ≥ 7.5 达标** (8.2), 与 qwen-2.5-72b (8.45) 同处候选区。
2. **4 过滤条件全过**: Capability 8.0 (DSL 生成可用, 恰达阈值) / Openness 10.0 (完全开源可训练) / Cost 电费级 (< $1/1M) / Latency P95 ≤ 100ms/token。
3. **tiebreaker (vs qwen-2.5-72b 8.45)**:
...
## 校正说明 (2026-09-23, Oracle bg_7fe026cc 审查发现)

> 本表初版 5 个候选的 `weighted_score` 全部存在**算术错误** (加权计算表达式正确, 但字段值抄录错误):
>
> | 候选 | 初版字段值 | 正确算术值 | 差异 |
> |------|:---:|:---:|:---:|
> | gpt-4-turbo | 6.0 | **5.8** | 2.7+1.4+0.8+0.9 = 5.8 |
> | claude-3-5-sonnet | 5.35 | **5.25** | 2.85+1.2+0.6+0.6 = 5.25 |
> | llama-3.1-70b | 8.0 | **8.2** | 2.4+1.0+1.8+3.0 = 8.2 |
> | qwen-2.5-72b | 8.25 | **8.45** | 2.55+1.1+1.8+3.0 = 8.45 |
> | deepseek-v2-chat | 7.5 | **7.35** | 2.55+1.4+1.6+1.8 = 7.35 |
>
> **关键影响**: `deepseek-v2-chat` 真实加权 7.35 < 7.5, 不满足 Weighted ≥ 7.5 过滤条件 — 初版 `passed_all_filters: true` 与过滤验证表 "✅ 7.5" 均为**假数据**。已修正为 `false` + "❌ 7.35 (Weighted < 7.5)"。
>
> **最终选择不变**: `llama-3.1-70b-instruct` 实际加权 **8.2 ≥ 7.5**, 4 过滤条件全过 — 选择结论与初版一致, 仅数值被校正。qwen 实际 8.45 仍为最高分, tiebreaker 论证保持不变。
>
> ADR-0078 D1 段同步修正 (commit hash 见该文件"算术校正"NOTE), 两处评分数据保持一致。
