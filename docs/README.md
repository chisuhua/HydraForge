...
| `adr-0078-finetune-base-model.md` | Fine-tune 基模与训练管线 (D1 4 维度评分 + D2 触发条件 + D3 数据 + D4 LoRA + D5 评估 + D6 AgenticMind 回流 + D7 serving) | ✅ Approved (2026-09-23 Wave 3 Phase 1 Pilot 激活 — D1/D3/D7 最小版 ship; D4-D7 Phase 2 延后, 派生自 ADR-0071 §D9) |
...
| `adr-0086-credit-assignment-contract.md` | 信用分配契约 (评估层 IEvaluator vs 归因层 AttributionRecord 划界; VersionPairDiff V1 归因方法 + ConfounderRecord 5 态混杂分层 (v1.1 +HarnessChange) + 4 态判定 Attributed/Confounded/Insufficient/NotAttempted; kMinBaselineSamples=5 Hotelling T² 经验值基线门控; judge_data_freshness() 数据时效性算法 stub 含 fast-path; GenomeVersion struct 单一所有权避免 ODR; 默认 fail-closed; S4 协同进化前置) | ✅ **Approved (v1.1, 2026-09-20)** (2026-08-31 self-evolution §七 #6 立项 → 2026-09-20 实施 ship: v1.0 首次实施基线 + v1.1 amendment HarnessChange confounder + judge_data_freshness 算法 + GenomeVersion 单一所有权; merge commit `886def1`, test_credit_assignment **12 cases / 40 assertions PASS** + `agenticdsl_evolution` 静态库; G16 Closed; C3 h-d-m-transition-guard unblocked; Phase 6c MetaRSI-v1 hard prerequisite) |
| `adr-0088-h-d-m-transition-guard.md` | H→D→M Transition Guard 架构 — MetaRSI-v1 关键规则强制 (5 态状态机 Idle/Harness/Data/Model/Done + can_transition() 编译期 5×5 矩阵 + evaluate_readiness() 三条件门控 (Attributed + 回归门 + 预算, 累积报告非短路) + reset_to_idle() 显式 API; D1-D4 + D7-D9 部分 ship per ADR-0088, D5 walk_ancestors + D6 judge_data_freshness 完整实装 + D8 主题注册 deferred to Sprint 34+ follow-up) | ✅ **Approved (v1.0, 2026-09-20)** (2026-09-20 rdd-arch 立项 → rdd-planner improvement 5-segment + planner-handoff v1.1 (recommended_route=complex) → rdd-builder P0 case 1 approve (auto-decision complex branch) → P2 实施 commit `0ffc637` → openspec archive commit `7a15744` (8 specs shipped, 4-file integrity PASS); test_transition_guard **13/13 cases / 47 assertions GREEN** + `agenticdsl_evolution` 静态库扩展 transition_guard.cpp; Oracle dual-agent review ship-with-fixes 6 Critical 修正应用 per commit `421fa62`; **Oracle post-ship verdict ALIGNMENT SCORE 62 / NEEDS_FIX** (2 critical + 4 major + 4 minor debt), 2 Sprint 34+ follow-up 已正式登记: `2026-09-20-ig-genome-registry-walk-ancestors` (D5/D6/D9) + `2026-09-20-adr-0068-appendix-a-evolution-themes` (D8 主题注册); Phase 6c MetaRSI-v1 C3 v1.0 ship, C4 harness-rsi-pilot unblocked) |
