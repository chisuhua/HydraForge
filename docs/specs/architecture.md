# AgenticOS 架构规范（五层模型，原 Agent-as-Plugin 架构 v1.2）

**日期**: 2026-07-22（2026-07-31 晋升为本 spec，D1 决议）
**状态**: ✅ Approved — **spec 契约层**（四层服务接口架构确立，含 Temporal Agent PoC 对齐；ADR-0067 追溯性正式化）
**作者**: Architecture Working Group
**关联**: ADR-0019 ~ ADR-0051, `docs/archive/specs/architecture-v2.2.md`（前代八层规范，已归档）, `docs/architecture/agent-evolution-pipeline.md`, `docs/architecture/application-layer-sota-positioning-v2.md`, `openspec/changes/pkgm-temporal-agent/`

**前置文档**: `docs/archive/architecture/agent-as-plugin-architecture.md` v1.0 → v1.1 (2026-07-16)；`docs/archive/specs/architecture-v2.2.md`（八层规范，2026-07-31 归档）

> **文档性质说明**（2026-07-31 D1 决议）：本文档自 `docs/architecture/agent-as-plugin-architecture-v1.2.md` 晋升为 specs 契约层架构规范，替代已归档的八层规范 v2.2。§1-§2 为规范性内容（五层职责 + R1~R5 层间依赖硬约束）；其余章节为分析性/原理说明（spec 允许含 rationale）。
...
> 八层规范 v2.2 已于 2026-07-31 归档至 `docs/archive/specs/architecture-v2.2.md`（D1 决议），本表保留作历史映射参考。

| 本架构层 | 对应 archive/specs/architecture-v2.2.md |
|---------|-------------------------------|
