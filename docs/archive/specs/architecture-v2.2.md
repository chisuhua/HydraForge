...
> ⛔ **Superseded（2026-07-31 归档，D1 决议）** — 被 [`docs/specs/architecture.md`](../../specs/architecture.md)（五层架构规范，原 agent-as-plugin-architecture-v1.2）替代。
>
> **成分处置说明**：
> - 八层分层模型（成分 A）→ 由五层模型 (L0~L4 + R1~R5, ADR-0067) 承接；
> - Layer Profile / 执行边界契约 / 沙箱分层（成分 B）→ 活的事实源为 ADR-0004 §8 + ADR-0031，本文档不再维护；
> - "第二大脑"产品愿景（成分 C：Persona Core / Contract System / ZK 信用账本 / brain-frontend / App Market，**D1b**）→ **自 2026-07-31 起正式归档，不构成当前路线图的承诺**；`llm_generate_dsl` 原语引用随之关闭（SPECS-ALIGNMENT 遗留债务清零）。

**文档版本：** v2.2.0  
**日期：** 2026-02-22  
**状态：** ⛔ Superseded（2026-07-31 归档）  
