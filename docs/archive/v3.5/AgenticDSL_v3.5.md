...
resources:
  ...
  - type: knowledge_graph
    capabilities:
      - multi_hop_query          # 支持多跳查询
      - evidence_path_extraction # 支持证据路径提取
      - subgraph_write           # 支持子图写入
    backend_hint: "gfm-retriever"  # 仅提示，非强制
    version_hint: "1.0+"           # 版本范围提示
```
...
> 注：资源声明**不替代**节点级 `permissions`，而是其前置契约。
...
> 例如：若未声明 `tool: web_search`，则任何包含 `permissions: [tool: web_search]` 的节点将无法通过调度前检查。
...
> AgenticDSL v3.5 是迈向 **AI 原生操作系统** 的坚实一步。  
