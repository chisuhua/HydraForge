...
| Phase 2 异步+EventBus | 🟡 Partial | 🟡 **ADR-0030 V2 仍 🔍 Proposed** (C2 ship 实施 2026-07-31 但 ADR 文件状态行未同步;§决策 2 P1-P4 实际状态由 AGENTS.md line 135 描述, ADR §最后更新 2026-06-26 待更新) | 2-3 周 | Phase 1 |
| Phase 3 执行策略+安全 | 🟡 Partial | 🟡 **C3+C4+C5+C6 全部 ship + archive** (57/57 ctest pass). **ADR-0031 文件本身仍 🟡 Partial** (§决策 8 明确 defer 4 项至 C6 ADR-0004 V2: cost_estimate ↔ IBudgetController / timeout_ms std::async / meta.min_layer 强制语义 / IExecutionPolicy::get_layer 路由钩子;P3-P4 active 内容已实施但 ADR 状态行未更新) | 3-4 周 | Phase 2 |
| Phase 4 模型路由 | 100% ██████████ | ✅ shipped (C7 2026-07-02, 3 策略 Plugin .so + ModelRegistry + examples 升级, ADR-0034 🔍 → ✅, 61/61 ctest) | 2-3 周 | Phase 3 |
| Phase 4.5 MVP清理 | 100% ██████████ | ✅ **已完成 (2026-07-03, C8 ship)** — SimpleCognitiveOrchestrator @internal 标记 + TODO(mvp) 清理 + examples/ 目录梳理 | 1-2 天 | Phase 4 |
...
| 2026-07-31 | **C3 Sprint 13 ship (ADR-0031 P1+P2 完整)** | 1 天 | ✅ **39/39 通过 (100%), 2 新测试文件 / 37 新 assertions** | **IExecutionPolicy 5-method 接口实施 (Oracle session `ses_0faa4dabeffeHGFoLdXE7AqwH7`)**：6 phase commits (230120e/acc7f55/76f52f9)。**关键 ship**: (1) **IExecutionPolicy 5-method 接口** — Oracle 决议落地 (requires_approval/should_execute/can_skip/get_layer/request_approval), 替代 8-method stub; (2) **PlanPolicy/AgentPolicy(默认)/YoloPolicy** — 3 个 5-method 实现 + PolicyFactory; (3) **ModeConfig 值结构体** — per-mode 常量移出虚接口; (4) **ApprovalHandler sync callback** — 3 种 transport (stdin/event_bus/test_auto), 不造 EventBus request_id 基础设施 (Oracle 决议); (5) **ModeSwitchDialog** — YOLO 切换需用户确认 (ADR-0031 §决策 6); (6) **DSLEngine 集成** — `set_execution_policy(PolicyMode)`, NodeExecutor 执行前审批检查, 默认 auto-approve 零回归。ADR-0031 仍 🟡 Partial (C3 ship P1-P2 实施;ADR 状态行未反映;§决策 8 4 项 defer 至 C6 ADR-0004 V2)。ctest: 39/39 PASS (0 regression)。Active change 数: 6 → 5 (C3 ready to archive). Sprint 14 (C4 adr-0031-p3p4-toolcoordinator) 可立即启动。详见 `openspec/changes/2026-06-26-adr-0031-p1p2-execution-policy/` + `docs/superpowers/plans/2026-06-26-sprint-11-to-18-roadmap.md` §四 C3。 |
| 2026-07-31 | **C4 Sprint 14 ship (ADR-0031 P3+P4 完整)** | 1 天 | ✅ **41/41 通过 (100%), 2 新测试文件 / 55 新 assertions** | **ToolCoordinator + LayerProfile 实施 (Oracle session `ses_0ed4408faffeLv8VfrC0s5PzW7`)**：3 phase commits (2d70c87/a48e563)。**关键 ship**: (1) **ToolCoordinator** — Oracle Option C standalone middleware (非 IToolRegistry 装饰器, 否决 Option B 因 9-method 中 7 个无意义 forwarder), 内部流程 layer check → ApprovalHandler.process_request → audit invoked → registry.call_tool → audit completed; (2) **LayerProfile 复用** — 既有 enum 配合 ADR-0004 §8 矩阵 (Workflow=all / Thinking=ReadOnly / Cognitive=none), 新增 `parse_layer()`/`check_layer_permission()`/`to_string()` helper (case-insensitive parse + invalid_argument throw); (3) **ToolMetadata V2** — 扩展 `allowed_layers`/`cost_estimate`/`timeout_ms` 3 字段 (默认空/0.0/30000, C4 defer 至 C6 接 IBudgetController + std::async); (4) **Audit Log EventBus** — `tool.audit.{invoked,completed,denied}` topic, payload 塞 `ToolResult::meta` JSON, **args 仅 key 不 value** (defense-in-depth 防 secret 泄露); (5) **NodeExecutor 优先级链** — `tool_coordinator_` > `approval_handler_` > 直接 `call_tool()`, `set_approval_handler()` 标记 `[[deprecated]]` 零回归; (6) **DSLEngine 集成** — 构造 ToolCoordinator + 注入 NodeExecutor (镜像 C3 注入模式); (7) **ADR-0031 §决策 4/5/7/8** 新增 — Layer Profile 设计 / ToolCoordinator Option C / Audit Log 命名 / 4 项 defer 至 C6. ADR-0031 仍 🟡 Partial (C4 ship P3-P4 实施;§决策 8 明确 4 项 defer 至 C6: cost_estimate ↔ IBudgetController / timeout_ms std::async / meta.min_layer 强制语义 / IExecutionPolicy::get_layer 路由钩子)。ctest: 41/41 PASS (39→41, 0 regression). Active change 数: 5 → 4 (C4 ready to archive). Sprint 15 (C5 adr-0033-session-hierarchy 可并行启动 / C7 adr-0034-model-router-plugin PDK ready) 可启动。详见 `openspec/changes/2026-06-26-adr-0031-p3p4-toolcoordinator/` + `docs/superpowers/plans/2026-06-26-sprint-11-to-18-roadmap.md` §四 C4。 |
| 2026-07-03 | **C8 Sprint 18 ship (Phase 4.5 MVP 清理)** | 0.5 天 | ✅ **52/52 通过 (100%), 零回归** | **Phase 4.5 MVP 清理 (OpenSpec change `2026-06-26-phase-4-5-mvp-cleanup`)**：4 个 ship item: (1) **SimpleCognitiveOrchestrator @internal 标记** — `include/agenticdsl/cognitive/simple_orchestrator.h` + `src/modules/cognitive/simple_orchestrator.cpp` 注释更新, 删除 `TODO(mvp)` 标记, 标记为 Phase 0 单轮 ReAct 编排器 — Phase 1+ 由 CognitiveWorker + ReactLoop 封装; (2) **TODO(mvp) 全部清除** — `grep -r "TODO(mvp)" src/ include/` 返回 0 results; (3) **examples/ 目录梳理** — 新建 `examples/README.md` (8 个 entry 说明: 6 C++ 编译示例 + 2 参考文档), 零删除; (4) **文档同步** — `docs/roadmap-status.md` Phase 4+4.5 标记 100%. 零 breaking change (仅注释+文档). OpenSpec change 准备 archive. 详见 `openspec/changes/2026-06-26-phase-4-5-mvp-cleanup/` + master plan C8。 |
| 2026-07-03 | **C9 Sprint 19 ship (ADR impl-scope audit)** | 2 天 | ✅ **54/54 通过 (验证后确认), 零回归** | **ADR 实施范围审计 (OpenSpec change `2026-07-03-phase4-5-impl-scope-audit`)**：C9 Phase 4.5 → Phase 5 过渡审计. 关键 ship: (1) **11 个 impl-scope audit 文档** 创建: 为 ADR-0001/0003/0004/0005/0007/0008/0019/0020/0022/0023/0033 每个创建 `-impl-scope.md` 文件，包含完整 Shipped/Evolved/Deferred 分类表; (2) **ADR 状态校准**: 所有 11 个 ADR 保持现有状态 (✅ 或 🟡)，确认无核心契约缺失; (3) **文档同步**: docs/README.md 添加 audit 文档索引 + roadmap-status.md Phase 4.5→5 过渡标记 + AGENTS.md §Recent Changes 更新; (4) **零代码修改**: 纯文档变更. 详见 `openspec/changes/2026-07-03-phase4-5-impl-scope-audit/`。 |
