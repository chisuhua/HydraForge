...
| 02 | [推理标准库规格](inference-stdlib/02-specification.md) | lib/inference/ 每个子图的完整规格 | [specs/stdlib-v3.10.md](../specs/stdlib-v3.10.md) |
...
| 03 | [标准库扩展清单](language-extensions/03-standard-library.md) | 三批扩展计划 + 编写规范 | [specs/stdlib-v3.10.md](../specs/stdlib-v3.10.md) |
...
### `docs/adr/`（引擎级 ADR）vs `docs/proposals/`（语言演进）

| 维度 | `docs/adr/` | `docs/proposals/` |
|------|-------------|-------------------|
| 层次 | 引擎实施级 | 语言设计级 |
| 状态 | ✅ 已批准，已实现 | 📝 草案，待讨论 |
...
| 关系 | 下层约束 — proposals 设计不得破坏现有 adr | 上层提案 — 依赖现有 adr 提供的基础能力 |

**一句话**：`docs/adr/` 说"引擎是如何实现的"；`docs/proposals/` 说"语言应该往哪个方向演化"。

### `docs/specs/`（引擎规约）vs `docs/proposals/language-extensions/`（语言扩展）

| 维度 | `docs/specs/` | `docs/proposals/language-extensions/` |
|------|---------------|----------------------------------------|
| 时效 | 当前有效（v3.10） | 未来提案 |
...
**一句话**：`docs/specs/` 是**基线**（现在能做什么）；`docs/proposals/language-extensions/` 是**增量**（将来能做什么）。
