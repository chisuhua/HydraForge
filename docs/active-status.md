...
| 维度 | 状态 |
|------|------|
| **Total ctest** | **244/244** 配置总数 (2026-09-17 持平 244: C1 `loop-agent-tools` 复用 `test_loop_agent_plugin` binary 增加 15 new test cases 但不增 binary 数, Case 1-15 + 1 E2E = 22 TEST_CASE / 95 assertions PASS); baseline 自 2026-09-12 232 → 244 (+12: Day 1-2 TSan residual + WorkflowCallbackChannel dtor barrier 回归守卫 + chat-session-static-logger-injection 6 cases + 2026-09-16 Day 3 实弹验证 8 真实 LLM tester binaries + C0 5 cases) |
