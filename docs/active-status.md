...
| 维度 | 状态 |
|------|------|
| **Total ctest** | **107/107 ✅** PASS (2026-08-01 实测: `cd build && ctest` → "100% tests passed, 0 tests failed out of 107"; 含 tf-integration-coverage 12 新 case — 11 active + 1 `[.disabled]` 依赖边派发待 follow-up; test_llm_provider_propagation 偶发 Subprocess aborted, 重跑即过, 定性 flaky) |
