...
| 维度 | 状态 |
|------|------|
| **Total ctest** | **251/251** 配置总数 (`ctest -N` 实测 2026-09-20, post C3 follow-up ship: 248/248 baseline + test_transition_guard + test_genome_walk_ancestors + test_credit_assignment). 2026-09-20 C3 h-d-m-transition-guard SHIPPED + C3 follow-up `ig-genome-registry-walk-ancestors` SHIPPED (test_genome_walk_ancestors 10 cases / 55 assertions GREEN; test_transition_guard 13 cases / 47 assertions; test_credit_assignment 12 cases / 40 assertions) |
...
| **Master Plan 更新** | `docs/roadmap/2026-09-16-pdk-chat-demo-evolution-roadmap.md` 2026-09-20 追加 C3 ✅ SHIPPED row + C4 🟡 Ready (unblocked) + ADR-0086 v1.1 / ADR-0088 v1.0 翻牌 + canonical spec sync 3 处 + ctest 248→251 + §十 Drift Log +2 行 + §十一 Adjustment Log +3 行 + 附录 B.3 C3 + follow-up 实施路径 (含 4 Oracle sessions) + Last Updated 2026-09-20. C4 harness-rsi-pilot unblocked (待 D8 主题注册 ship 后 Go/No-Go 门). |
