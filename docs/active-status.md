...
| 维度 | 状态 |
|------|------|
| **Total ctest** | **252/252** 配置总数 (`ctest -N` 实测 2026-09-21, post C4 harness-rsi-pilot ship: 251/251 baseline + test_harness_rsi_pilot binary 新增 8 cases / 39 assertions). 2026-09-21 C4 `harness-rsi-pilot` SHIPPED + GO (Decision Record `docs/audits/2026-09-21-harness-rsi-pilot-go-no-go.md`, 5 判据全绿, 4 摩擦 Wave 3 优先解决). focused ctest 8/8 PASS (harness_rsi_pilot + C3 ship 5 + DB1 test_tool_registry 三件套) 0 new regression |
...
| **TSan** | 超时跳过 (机器性能受限) |
| **OpenSpec active** | **7** (2026-09-20: C2 `genome-registry` archived 2026-09-19 + C3 `h-d-m-transition-guard` archived 2026-09-20 + C3 follow-up `ig-genome-registry-walk-ancestors` archived 2026-09-20, 释放 2 slot; new D8 follow-up `adr-0068-appendix-a-evolution-themes` registered). 累计: harness-rsi-pilot(C4) + P1 + fix-generate-subgraph-static-next + chat-real-llm-coverage-phase-h + fix-flatten-layers-comment-drift + provider-llm-tool-empty-passthrough + adr-0068-appendix-a-evolution-themes = **7 active**.<br/>**2026-09-20 C3 + C3 follow-up archive**: 10 atomic commits `a40e9e1`→`231cd8d` (C3 follow-up) + 4 C3 baseline commits (`0ffc637`+`94f4ab4`+`421fa62`+`7a15744`). Oracle dual-agent pre-impl review (bg_7984922b + bg_dd35a52d) + Oracle post-impl SHIP-with-fixes verdict 80/100 (bg_f6190442) + Oracle 2nd review APPROVE 95/100 (bg_86a511e0). 累计: 10 + 2 = 12 changes archived, 7 active. |
