...
| **TSan** | 超时跳过 (机器性能受限) |
| **OpenSpec active** | **4** (2026-09-22: Pre-Wave3 Plan §2 4-Gate 序列 G1+G2+G3 G4 全部 SHIPPED ✅. G2 evolution-verdict-reward-quality merge dc12a17 (Oracle bg_ebfe1c25 SHIP verdict 0C + 2 Minor). 4 active: chat-real-llm-coverage-phase-h + provider-llm-tool-empty-passthrough + fix-flatten-layers-comment-drift + fix-generate-subgraph-static-next + intent-classification-router. 24h cooling-off 计时中, 起点 2026-09-22 22:30 (G2 merge dc12a17), 期间可做 Wave 3 立项准备, 冷却期满后正式立项 ADR-0078 Model-RSI pilot) |
