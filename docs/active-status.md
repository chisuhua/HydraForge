...
| 维度 | 状态 |
|------|------|
| **Total ctest** | **252** 测试总数 (`ctest -N` 列数 2026-09-21, post C4 harness-rsi-pilot ship: 251/251 baseline + test_harness_rsi_pilot binary 新增 9 cases / 43 assertions). **注意**: 含 4 项 pre-existing failures (test_chat_session_events / test_budget_alert / test_e2e_real_llm / test_skill_interpreter 7.S29-1), git stash 验证与 C4 无关, 不计入回归. 2026-09-21 C4 `harness-rsi-pilot` SHIPPED + GO (Decision Record `docs/audits/2026-09-21-harness-rsi-pilot-go-no-go.md`, 5 判据全绿, 4 摩擦 Wave 3 优先解决). focused ctest 8/8 PASS (harness_rsi_pilot + C3 ship 5 + DB1 test_tool_registry 三件套) 0 new regression |
...
| **TSan** | 超时跳过 (机器性能受限) |
| **OpenSpec active** | **3** (2026-09-25 update: `fix-flatten-layers-comment-drift` archived per Pattern #10 hygiene — 5/5 Acceptance 中 4 项已由 F1 SHIP (2026-09-18, commits `a96842e` + `9dc3ac8`) 闭环, 仅 D3 (AGENTS.md Pattern #1 step 4 sub-bullet) 转为 `.rddf/improvements/fix-flatten-layers-comment-drift.md` follow-up tracker. 详情见 archived proposal.md Cancellation Note. 历史 context 2026-09-22: Pre-Wave3 Plan §2 4-Gate 序列 G1+G2+G3 G4 全部 SHIPPED ✅. G2 evolution-verdict-reward-quality merge dc12a17 (Oracle bg_ebfe1c25 SHIP verdict 0C + 2 Minor). 3 active: chat-real-llm-coverage-phase-h + provider-llm-tool-empty-passthrough + fix-generate-subgraph-static-next + intent-classification-router. 24h cooling-off 计时中, 起点 2026-09-22 22:30 (G2 merge dc12a17), 期间可做 Wave 3 立项准备, 冷却期满后正式立项 ADR-0078 Model-RSI pilot) |
| **Master Plan 更新** | `docs/roadmap/2026-09-16-pdk-chat-demo-evolution-roadmap.md` 2026-09-21 追加 C4 ✅ SHIPPED + GO row + D8 主题注册 ✅ SHIPPED + ADR-0086 v1.1 / ADR-0088 v1.0 / ADR-0068 v2.2 翻牌 + ctest 251→252 + §三 Overview C4 ✅ SHIPPED + §四 C4 子节 TODO 全 ✅ + §十 Drift Log +1 行 + §十一 Adjustment Log +1 行 + 附录 B.4 C4 + follow-up 实施路径 (含 4 Oracle sessions + Decision Record 闭环) + Last Updated 2026-09-21. Wave 3 (ADR-0078 Model-RSI pilot) 立项依据已就绪 (Go rationale: 5 判据全绿 + 4 摩擦 Wave 3 优先解决). |
