...
| **31** | **Distillation Data Plane V1 (CaptureMode 三态枚举 + IDistillationWriter 契约 + FileDistillationWriter V1 默认实现 + TrajectoryIR → DistillationRecord 桥接 + payload redact hash-only PII 防御)** | `include/agenticdsl/types/capture_mode.h` (47行 ✅) + `include/agenticdsl/types/distillation_record.h` (77行 ✅) + `include/agenticdsl/contract/idistillation_writer.h` (59行, 3虚函数+工厂 ✅) + `src/modules/distillation/file_writer.cpp` (Phase 1) + `src/modules/distillation/trajectory_bridge.cpp` (Phase 2) | **Phase 0 ship ✅** test_capture_mode **4 cases / 22 assertions PASS** (commit `11d3515`); **Phase 1 待 ship**: test_distillation_writer 5 + test_event_log_capture_mode 5 = ≥10 cases; BREAKING EventLogConfig `bool capture_prompt_bytes → CaptureMode capture_mode` (5 消费者迁移, `grep capture_prompt_bytes = 0` 强验证); 既有 **20 个** contract 头文件零修改 (Oracle B3, 修正 D8) | **ADR-0080 v1.2** + **ADR-0061-13** | **Phase 0 ✅ ship 2026-08-29 (commit `11d3515`), Phase 1-3 待启动 (Sprint 24 W2-Sprint 25)** |
