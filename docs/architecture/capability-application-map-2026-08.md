...
## 一、架构能力清单（**33 项**已 ship 能力，L4 含 #23 T14 + #24 T17 + #25 T15 + #26 IEvaluator V2 v2.0 后置增补 + #27 GEPALoop + #28 Prompt Evidence Gate + #29 MCTSWorkflowSearch + #30 Cross-Cutting Pattern PDK V1 + #31 Distillation Data Plane V1 (Change #1 ship 后增补, ✅ Shipped 2026-08-29) + #32 Credit Assignment Contract v1.0+v1.1 (✅ Shipped 2026-09-20, merge commit 886def1) + **#33 H→D→M Transition Guard v1.0 (Phase 6c MetaRSI-v1 C3, ✅ Shipped 2026-09-20, merge commit 7a15744, Oracle NEEDS_FIX verdict ALIGNMENT SCORE 62, 2 follow-up registered)**）
...
### L4 可观测 + 治理层（14 项，#20 EventLog / #21 Session 4-scope / #22 17 ErrorCode / #23 T14 行为回归 / #24 T17 SkillCompiler / #25 T15 Trajectory IR / #26 IEvaluator V2 / #27 GEPALoop / #28 Prompt Evidence Gate / #29 MCTSWorkflowSearch / #30 Cross-Cutting Pattern PDK V1 / #31 Distillation Data Plane V1 / #32 Credit Assignment Contract v1.0+v1.1 / **#33 H→D→M Transition Guard v1.0**）
...
| **33** | **H→D→M Transition Guard v1.0 (5 态状态机 + can_transition 编译期矩阵 + evaluate_readiness 三条件门控 (Attributed + 回归门 + 预算, 累积报告非短路) + reset_to_idle() 显式 API; D1-D4 + D7-D9 部分 ship per ADR-0088, D5 walk_ancestors + D6 judge_data_freshness 完整实装 + D8 主题注册 deferred to Sprint 34+ follow-up)** | `include/agenticdsl/evolution/transition_guard.h` (~145 行, D1-D4 + D7 inline impl + D8 字符串常量 + D9 stub plan) + `src/evolution/transition_guard.cpp` (~30 行, D8 event schema constants) + `src/evolution/CMakeLists.txt` (transition_guard.cpp 静态库注册) + `tests/test_transition_guard.cpp` (~230 行, 13 cases / 47 assertions GREEN) + `tests/CMakeLists.txt` (test_transition_guard link agenticdsl_evolution) | test_transition_guard **13/13 cases / 47 assertions PASS** (AC-1/AC-2/AC-3/AC-4); test_credit_assignment **12/12 cases / 40 assertions** 零回归 (ADR-0086 v1.1 baseline 保持); ctest 248 PASS 零回归; 7 atomic commits per pattern #4 (arch ADR 0703ecd + planner 9bbfb04 + Oracle fixes 421fa62 + feat 0ffc637 + status flip 94f4ab4 + archive 7a15744 + governance 补完 governance-debt-cleanup); Oracle dual-agent review ship (6 Critical 修正应用 per commit `421fa62`: Q1 event topic + Q3 signature 5-arg + Q6 walk_ancestors default impl + Q4 harness in GenomeSpec + Q1 failure semantics + Q3 fail-closed 分层); **Oracle NEEDS_FIX verdict ALIGNMENT SCORE 62** (2 critical + 4 major + 4 minor debt, Sprint 34+ follow-up 已注册: `2026-09-20-ig-genome-registry-walk-ancestors` + `2026-09-20-adr-0068-appendix-a-evolution-themes`) | **ADR-0088 v1.0** (✅ Approved 2026-09-20) | **Sprint 25+ Phase 6c MetaRSI-v1 C3 (2026-09-20 ship, merge commit 7a15744, 8 specs archived)** |

**总覆盖**: **33 项已 ship 能力 (含 #31 Distillation Data Plane V1 ✅ Shipped 2026-08-29 + #32 Credit Assignment Contract v1.0+v1.1 ✅ Shipped 2026-09-20 + #33 H→D→M Transition Guard v1.0 ✅ Shipped 2026-09-20, merge commit 7a15744)** = L0(5) + L1(5) + L2(4) + L3(5) + L4(14)
...
**v2.6 新增**: L4 +1（H→D→M Transition Guard v1.0 — ADR-0088 首次实施, C3 h-d-m-transition-guard ship, 5 态状态机 + 编译期迁移矩阵 + 三条件门控 (Attributed + 回归门 + 预算) 累积报告非短路 + reset_to_idle() 显式 API; Oracle dual-agent review ship (6 Critical 修正应用) + openspec archive (8 specs) + Oracle post-ship verdict ALIGNMENT SCORE 62 (2 critical + 4 major + 4 minor debt, 2 Sprint 34+ follow-up 已正式登记: `2026-09-20-ig-genome-registry-walk-ancestors` + `2026-09-20-adr-0068-appendix-a-evolution-themes`)）
...
| 2026-08-29 | **v2.5+** | **Phase 0 ✅ ship (commit `11d3515`) + 文档同步收口** | (1) **Phase 0 ship**: 4 files changed +267 行 (capture_mode.h 47 + distillation_record.h 77 + idistillation_writer.h 59 + test_capture_mode.cpp 84); **4 cases / 22 assertions PASS** (含 phase0_headers_compile_smoke 防零编译覆盖); (2) **Oracle/Metis 双审查 9 项修正全部生效**: D1 (reward_signal.h 路径 types/), D2 (TDD Verify-fail cmake+make), D3 (3 虚函数+1 工厂对齐 ADR-0061-13 §决策 3), D4 (零编译覆盖), D5 (commit 前/后双向验证), D6 (DistillationRecord 字段全集), D7 (RewardSignal 真嵌入), D8 (20 个 contract 文件清单), D9 (test 引用修正); (3) **Oracle B3 验证**: commit 前 `git diff HEAD -- include/agenticdsl/contract/` 0 行, commit 后 `git diff HEAD^ HEAD -- include/agenticdsl/contract/` 仅含 idistillation_writer.h +59 行, 其余 20 文件 0 行; (4) **ADR-TRACKING-01**: WARNING 34 与 baseline 持平 (本 commit 零 docs/ + openspec/ 修改); (5) **修正版 prompt 创建**: `.rddf/plans/capture-mode-and-distillation-writer-v1-phase-0-CORRECTED.md` (655 行, Oracle+Metis 9 项修正); (6) **cap-map #31 状态**: 🔵 Active → ✅ Phase 0 ship 2026-08-29; (7) **ADR 头部翻转**: ADR-0061-13 + ADR-0080 v1.2 amendment `⏳ 代码 ship: Pending` → ✅ code ship; (8) **spec.md 同步**: D3 (3 虚函数+工厂) + A3 (Scenario 默认值 Off 实现注记); (9) **tasks.md 同步**: Phase 0 7 个 task 全部勾选; (10) **遗留**: ⚠ cooling-off 实际违反（kickoff 10:55 → commit 17:58, 仅 7h3m vs 应 24h+）, kickoff 追溯 note 已追加; (11) 头部 v2.5 维持 + 最后验证 2026-08-29 |
| 2026-08-29 | **v2.5.1** | **Change #1 完整 ship + archive (capture-mode-and-distillation-writer-v1)** | (1) Phase 0-2 完整 ship (3 commits: 11d3515 + 9a781f8 + ed5fcaf); (2) #31 Distillation Data Plane V1: 🔵 Active → ✅ Shipped (21 cases PASS); (3) ADR-0080 v1.2 + ADR-0061-13 头部 → ✅ 完整 ship + tracking: shipped; (4) ADR-0068 附录 A v1.7: event_log.capture_mode_downgrade 主题注册; (5) OpenSpec change archived 2026-08-29; (6) 头部 v2.5 维持 + 最后验证 2026-08-29 |
**后续追踪**:
...
| **T26**: ~~✅ **SHIPPED** 2026-08-28~~ — Cross-Cutting Pattern PDK V1 (ADR-0085) | 2.2 sprint → 实际 1 周 ✅ | 横切功能管理 PDK 模式 | 全部应用类型的横切关注点管理 | 6 层抽象扩展点全部 ship + ADR-0085 ✅ Approved | T26 (新增) → OpenSpec `pdk-cross-cutting-patterns` |
