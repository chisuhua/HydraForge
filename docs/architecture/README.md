...
| `capability-application-map-2026-08.md` | 架构能力-应用地图（**32 项已 ship 能力** + 9 项 open gap + 17 类应用场景 + 23 个工程任务 T1-T22 + T26 解锁映射 + Phase 6c MetaRSI-v1 关键路径 (C2 ADR-0086 ✅ / C3 h-d-m-transition-guard unblocked / C4 harness-rsi-pilot 待填) + Oracle 评审蒸馏+自进化专题 §八） | ✅ Active (v2.6.0) | 2026-09-20 | 新增能力 ship / open gap 闭合 / 新应用类型立项 / Oracle 评审输入 / 自进化架构边界与 T14-T22 推进 / ADR-0083 状态修正 + ADR-0084 文件创建 / Sprint 24 审计闭环 5 commits (20735dc/29c5b7c/45db64f/5e0bc7c/07b81ca) / ADR-0086 v1.0+v1.1 ship 5 commits (5819f55+9deca7d+53396cf+798b6c6+886def1) |
| `self-evolution-architecture-2026-08.md` | **自进化架构 Source of Truth v1.5**（9 段闭环 + 4 支撑平面 + 3 阶段路线 + S0-S4 promotion criteria；v1.5 升档 2026-09-23 反映 11 项 ship + C2/C3/C4 + Pre-Wave3 4-Gate + Wave 3 Phase 1；**§十二 Verification Matrix** 反向指标门 (R8) + 反作弊三模式 (R9) + **§十二.9 上下文驱动约束 (R13)**） | ✅ **Approved (v1.5)** | 2026-09-23 | ADR 状态翻转 / Pre-Wave3+Wave 3 阶段 ship / "超出 v1.5 范围"立项 / 配套 harness-architecture / rsi-architecture 三方同步 / R8+R9+R13 ship gate |
| `harness-architecture-2026-09.md` | **Harness 架构 Source of Truth v1.0**（Harness 作为自进化核心变异对象的数据模型 + 装配 + 变更能力 (apply_harness_mutation) + 5-tier 守门 (Gate 0/1/2/2.5/3 + H→D) + IGenomeRegistry 持久化纵深内容；**§十二 Verification Matrix** H1-H6 红线 + 5-tier gate 反向校验 + **§十二.9 上下文驱动约束 (R13)** + trace JSONL 必含 `meta.context_id`） | ✅ **Approved (v1.0)** | 2026-09-23 | C4 / G1 / G4 后续 ship / 新 Gate tier / 新 Harness 事件 / self-evolution / rsi 同步 / R8+R9+R13 ship gate |
| `rsi-architecture-2026-09.md` | **RSI 架构 Source of Truth v1.0**（原 `rsi-three-operators-hydraforge-mapping-2026-09.md` 2026-09-18 调研 + 2026-09-23 research → architecture 升档；Harness/Data/Model 三算子叠加策略 + 5 阶段 ship 实施路径 + 2 阶段未 ship 候选立项；**§十二 Verification Matrix** R1-R4 真 RSI 三判据 + 反作弊三模式 + **§十二.9 上下文驱动约束 (R13)** + **§11.8.8 上下文驱动治理**） | ✅ **Approved (v1.0)** | 2026-09-23 | Data-RSI / Harness-RSI / Model-RSI 阶段 ship / S4 立项 / Wave 3 Phase 2 立项 / self-evolution / harness 同步 / R8+R9+R13 ship gate |
| ⚠️ **三方 SoT 基线声明**（2026-09-23 升级）：**HydraForge 自进化方向架构以 `self-evolution-architecture-2026-08.md` v1.5 + `harness-architecture-2026-09.md` v1.0 + `rsi-architecture-2026-09.md` v1.0 三方一致为 Source of Truth**。每份含 **§十一 pdk_chat_demo traceback** (静态) + **§十二 Verification Matrix** (动态判据 + 红线) + **§十二.9 上下文驱动约束** (R13 ContextRequest 触发)。任何变更任一方必须同步另外两方边界段 + AGENTS.md "Reverse Indicator Rule" commit 字段 (per Single-Developer Mode)。 | — | — | — | — |
| 📋 **SoT 治理四件套配套**: 上述 3 份 SoT + [AGENTS.md §REVERSE INDICATOR RULE](../AGENTS.md) (commit 强制 [Reverse Indicator] 段 + R13 context_ids) + [`openspec/changes/pdk-chat-demo-evolution-reference-example/`](../openspec/changes/pdk-chat-demo-evolution-reference-example/) (L2 spec R8 + R9 + **R13 ContextRequest 零 hardcode**) = Source of Truth 完整治理闭环。 | — | — | — | — |
| 📐 **L2 零 hardcode 契约** (per R13): `examples/pdk_chat_demo_evolution/` 是 reference example 入口 (UI/CLI/5-tier gate/trace JSONL), **不**是 autonomous evaluator. 任何 harness-rsi / data-rsi / model-rsi 验证 **必须** 由用户提供 ContextRequest (--context-file flag, ≥ 3 类 code/research/debug) 触发. L2 ship 时附 `examples/pdk_chat_demo_evolution/fixtures/contexts/{code,research,debug}-class-context.jsonl` 3 reference file. | — | — | — | — |
...
| 2026-09-20 | **v2.6.0 / Phase 6c MetaRSI-v1 关键路径 ship**: ADR-0086 v1.0+v1.1 ✅ Approved (2026-09-20) — `agenticdsl::evolution` 命名空间 `attribution_record.h` + `attribution_version_pair_diff.h` + `src/evolution/` (新增 `agenticdsl_evolution` 静态库) + `tests/test_credit_assignment.cpp` 12 cases / 40 assertions PASS + 4 atomic commits (5819f55 feat + 9deca7d docs ADR + 53396cf docs C3 + 798b6c6 chore archive) + merge commit `886def1`。`capability-application-map-2026-08.md` §一 +#32 (31→32 项能力, L4 12→13, v2.4+v2.5 新增行) + §六 6.1 验证附录 + ADR-0086 grep 验证命令。`self-evolution-architecture-2026-08.md` §1.3 信用分配契约 + §五 ADR-0086 行 + §七 #6 Approved 翻牌 + 头部 v1.2→v1.3。`docs/README.md` ADR 表 ADR-0086 行状态翻牌 🔍 Proposed → ✅ Approved (v1.1, 2026-09-20) + 字段扩展 (HarnessChange + kMinBaselineSamples + judge_data_freshness + GenomeVersion)。**G16 Closed, C3 h-d-m-transition-guard unblocked, C4 harness-rsi-pilot 待填**。 |
