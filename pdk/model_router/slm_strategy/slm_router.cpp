...
// pdk/model_router/slm_strategy/slm_router.cpp
// 功能描述：SLMModelRouter Plugin 入口 (T16, ADR-0061-04)。
//          export extern "C" pdk_register_tools(IToolRegistry&),
//          注册 model_router/slm 工具: lambda 解析 args → SLMModelRouterPolicy::route()。
//          遵循 PDK Plugin 契约 (与 cost/quality/latency 一致)。
...
// 从 json args 解析 RoutingContext (与 cost/quality/latency 一致)
...
// 从 json args 解析候选模型列表 (与 cost/quality/latency 一致)
...
// Plugin 入口: 注册 model_router/slm 工具
extern "C" void pdk_register_tools(::agenticdsl::IToolRegistry& registry) {
  auto router = std::make_shared<agenticdsl::pdk::SLMModelRouterPolicy>();

  // 构造 ToolMetadata V2: name / description / domain / category / min_layer / approval
  ::agenticdsl::ToolMetadata meta{
    "model_router/slm",                         // name
    "SLM 优先模型路由: 在带 fast/slm tag 的候选中选最便宜的; fallback 到非 SLM",  // description
    "model_router",                              // domain
...
extern "C" const hydraforge::PluginInfo pdk_plugin_info = {
  ...
  "hydraforge_model_router_slm",                    // name
  1, 0, 0,                                          // semver 1.0.0
  "SLMModelRouter - SLM-first routing (NVIDIA 2025)",// description
  "model_routing,slm"                               // capabilities
};